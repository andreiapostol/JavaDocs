 ...
 *              keySet() method
 *              b) Find all the countries that contain 'Q' or 'q' letter, iterating through the keys of the Map using
...
public class Exercise1 {
    ...
    public List<String> iteratingOverKeys(){
        ...
        for (String k : countries.keySet()) if(k.startsWith("R")) seekingCountries.add(k);
    ...
    public List<String> iteratingOverEntries(){
        ...
        for (Map.Entry<String, String> k : countries.entrySet())
            if(k.getKey().contains("Q") || k.getKey().contains("q")) seekingCountries.add(k.getKey().toLowerCase());
    ...
    public String iteratingOverValues(){
        ...
        for(String v : countries.values()){
            if (v.length() > seekingCapital.length()) seekingCapital = v;
        }
