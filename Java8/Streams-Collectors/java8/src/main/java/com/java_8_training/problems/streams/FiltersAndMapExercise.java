...
public class FiltersAndMapExercise {
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter(a->a.equals(name)).collect(toList());
        return personName;
    }
    ...
    public static List<Integer> filterNull(){
        ...
        List<Integer> notNullInput = new ArrayList<>();
        //TODO #3 find all elements that are not null;
        notNullInput = input.filter(a->a!=null).collect(toList());
        return notNullInput;
    }
    ...
    public static List<Person> findPersonByAgeAndName(List<Person> personList, String name, Integer age){
        ...
        persons = personList.stream().filter(a->a.getName() == name && a.getAge() == age).collect(toList());
        return persons;
    }
    ...
    public static List<String> getOnlyNames(List<Person> personList){
        ...
        //TODO #6
        // HINT: use map
        collect = personList.stream().map(a->a.getName()).collect(toList());
        return  collect;
    }
    ...
    public static int findTheMinimumNumber(){
        ...
        int min = input.reduce(Integer.MAX_VALUE, (acc, x) -> Integer.min(acc, x));
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        int min = 0;
        min = personList.stream().map(person->person.getAge())
                .reduce(Integer.MAX_VALUE, (acc, age) -> Integer.min(acc, age));
        return min;
    }
    ...
    public static double findAgeAverage(List<Person> personList){
        double sum = 0;
        //TODO #13
        sum = personList.stream().map(person->person.getAge()).reduce(0, (acc, age)->acc + age);
    ...
    public static List<Nationality> findNationalityByName(List<Person> personList){
        ...
        persons = personList.stream().filter(a->a.getName()
                .startsWith("J")).map(a->a.getNationality()).distinct().collect(toList());
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        firstSquareDivisibleByThree =
                someNumbers.stream().filter(a->a%3 == 0)
                .filter(a -> Math.pow(Math.sqrt(a),2) == Math.pow((int)Math.sqrt(a),2)).findFirst();
    ...
    public static int sumStringLength(){
        ...
        int sum = 0;
        //TODO #16
        sum = strings.stream().map(x->x.length()).reduce(0, (acc, x) -> acc + x);
        return sum;
    }
    ...
    public static  int sumSalary(List<Person> personList){
        int sum =0;
        //TODO #17
        sum = personList.stream().map(a->a.getSalary()).reduce(0, (acc, a)->acc + a);
        return sum;
    }
